...
with st.sidebar:
    ...
    with st.expander("수입 항목 관리"):
        income_categories = st.session_state.data[selected_meeting].get("categories", {}).get("수입", [])
        st.write("현재 수입 항목:")
        for cat in income_categories:
            ...
            with col1:
                st.text(cat)
            with col2:
                if st.button("삭제", key=f"del_income_{cat}"):
                    st.session_state.data[selected_meeting]["categories"]["수입"].remove(cat)
                    save_data(st.session_state.data)
                    st.rerun()
        
        new_income_category = st.text_input("새 수입 항목 추가", key="new_income")
        if st.button("수입 항목 추가", key="add_income"):
