...
with tab1:
    ...
    with col2:
        ...
        if "categories" not in st.session_state.data[selected_meeting]:
            ...
            save_data(st.session_state.data)
            available_categories = st.session_state.data[selected_meeting]["categories"][transaction_type]
        
        # 드롭다운과 직접 입력 옵션
        category_option = st.radio("항목 선택 방법", ["목록에서 선택", "직접 입력"], horizontal=True, key=f"category_option_{selected_meeting}")
        
        if category_option == "목록에서 선택":
            if available_categories:
                category = st.selectbox("항목", available_categories, key=f"category_select_{selected_meeting}")
            else:
                st.warning("등록된 항목이 없습니다. 사이드바에서 항목을 추가하거나 '직접 입력'을 선택하세요.")
                category = ""
        else:
            category = st.text_input("항목", placeholder="예: 회비, 식사비, 간식비 등", key=f"category_input_{selected_meeting}")
